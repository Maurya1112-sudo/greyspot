...
logger = logging.getLogger("run_ucl_comparison_lr_sweep")
...
# source alone.
CANDIDATES = {
    # LEARNING RATE SWEEP for spatial_first (this project's winning
    # encoder order), added 2026-09-03. lr=0.01 has been used as the
    # FIXED default in every single experiment this session - it was
    # never itself swept. It was only ever used as a CONTROL value when
    # sweeping the *other* (temporal_first) encoder order, which found a
    # real lr x architecture interaction there (optimum at 2e-3, not
    # 1e-2). Nobody has checked whether 1e-2 is actually optimal for
    # spatial_first, or just the value nobody happened to question.
    # Five values bracketing the current default, fixed in advance.
    "lr=0.003": dict(heads=3, gat_layers=1, negative_binomial=False, lr=0.003),
    "lr=0.005": dict(heads=3, gat_layers=1, negative_binomial=False, lr=0.005),
    "lr=0.01 (current default)": dict(heads=3, gat_layers=1, negative_binomial=False, lr=0.01),
    "lr=0.015": dict(heads=3, gat_layers=1, negative_binomial=False, lr=0.015),
    "lr=0.02": dict(heads=3, gat_layers=1, negative_binomial=False, lr=0.02),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_lrsweep.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_lrsweep.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_lrsweep.csv")
