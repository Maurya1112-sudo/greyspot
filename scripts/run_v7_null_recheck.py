...
logger = logging.getLogger("run_v7_null_recheck")
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "v7_null_recheck_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "v7_null_recheck_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "v7_null_recheck_summary.csv")
