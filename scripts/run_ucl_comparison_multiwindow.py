...
CANDIDATES = {
    ...
    "heads=3 (validating on a second borough)": dict(heads=3, gat_layers=1, negative_binomial=False),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_metrics.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary.csv")
