"""MULTI-SEED ENSEMBLE on the real OS Open Roads network - variance
reduction as a lever in its own right, built 2026-09-02.

**The observation that motivated this**: every AccHR@20 figure this
project has ever reported - including all the confirmed, significant
ones - came from a SINGLE weight initialisation. `seed` defaults to 42
in `train_gat_temporal_walkforward` and no sweep script has ever varied
it. At 6-11 training instances per window, that is precisely the regime
where one seed's result is least trustworthy, and where averaging
several independently-initialised models is the standard, well-founded
fix. Independently corroborated by this session's own discovery that
training is not bit-reproducible on CUDA (one window of a re-run
identical config moved ~4 points) - real run-to-run variance exists and
is measurable.

Trains one model per seed in `ENSEMBLE_SEEDS`, averages their
PREDICTIONS (on both the held-out window and the calibration instance,
so the conformal interval stays consistent with the same ensemble
mean), then scores once on the averaged prediction. Deliberately NOT
`len(ENSEMBLE_SEEDS)` separate AccHR@20 scores averaged after the fact
- that would report the mean of noisy rankings rather than the ranking
of a less-noisy prediction, which is the thing actually wanted.

Otherwise identical to `run_ucl_comparison_multiwindow_os_open_roads.py`
(real OS Open Roads network with polygon clipping, POI +
socio-demographic features, heads=3, ZIP decoder, light protocol) -
only the ensembling changes, isolating this one variable against the
current best-known configuration.

Cost: `len(ENSEMBLE_SEEDS)`x the training time of the single-seed run
it is compared against. Real, accepted, not free.

Run from the project root: python scripts/run_ucl_comparison_multiwindow_os_open_roads_ensemble.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_ensemble")

# Multi-seed ensembling as variance reduction. Every AccHR@20 figure this
# project has ever reported came from ONE weight initialisation (seed=42,
# the default nobody ever varied), at 6-11 training instances per window -
# exactly the regime where a single seed is least trustworthy. Averaging
# several independently-initialised models' PREDICTIONS before ranking is
# the standard fix (averaging their metric values after the fact would
# not reduce ranking noise the same way).
ENSEMBLE_SEEDS = [42, 123, 7, 2024, 31337]  # 42 = every other script's default, kept so this is directly comparable
...
# source alone.
CANDIDATES = {
    "heads=3 + POI + socio-demographic + REAL OS Open Roads network, 5-seed ENSEMBLE": dict(
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        # Train one model per seed; average their PREDICTIONS (on both the
        # held-out window and the calibration instance, so the conformal
        # interval stays consistent with the same ensemble mean) before
        # scoring once - not N separate scores averaged after the fact,
        # which would not reduce the ranking noise the same way.
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_ensemble.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_ensemble.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_ensemble.csv")
