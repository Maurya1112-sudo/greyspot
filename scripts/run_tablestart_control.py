...
logger = logging.getLogger("run_tablestart_control")
...
# source alone.
# TABLE-START CONTROL, added 2026-09-04.
#
# **Why this exists.** The head-to-head run accidentally revealed that
# this project's "+15.85 points from long crash history" attribution is
# WRONG. With the SAME architecture and only short (7/14/30d) history
# features, the score is 78.94% - almost identical to the 79.44% with
# the full 30d..1825d ladder (5 of 6 windows bit-identical). The horizon
# is worth ~+0.5 points, not ~+16.
#
# The confound: when the feature table was extended back to 2021-01-01
# to support a 365-day rolling window, that ALSO silently fixed a latent
# data-quality problem. The original baseline's table began 2022-01-01,
# the same date the instance grid begins, so its earliest training
# instances had TRUNCATED rolling features - `rolling(min_periods=1)`
# returns a partial sum when fewer than `window` days of history exist.
# Roughly half of the 6-11 training instances per window were learning
# from malformed features.
#
# This script isolates that single variable: identical model, identical
# SHORT feature set, only the table start (and the collision years
# loaded to populate it) differ. Nothing else changes.
_LONG = ["collision_count_90d", "collision_count_365d", "collision_count_730d",
         "collision_count_1095d", "collision_count_1825d"]
_SHORT_ONLY = [c for c in FEATURE_COLUMNS if c not in _LONG]

CANDIDATES = {
    "table starts 2022 (= instance grid start; partial early rolling features)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=_SHORT_ONLY, _table_start="2022-01-01", _years=[2022, 2023, 2024],
    ),
    "table starts 2021 (complete early rolling features)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=_SHORT_ONLY, _table_start="2021-01-01", _years=[2021, 2022, 2023, 2024],
    ),
}
...
def build_instances(borough_name: str, table_start: str = None, years: list = None):
    borough = get_borough(borough_name)
    borough_slug = slug(borough.name)
    years = years or YEARS
    table_start = table_start or TABLE_START_DATE
    ...
    casualties = load_casualty_years([y for y in years if y >= 2021], RAW_DIR)
    ...
    table = build_segment_day_table(edges, daily_counts, graph, start_date=table_start, end_date=END_DATE)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    built = {}
    logger.info("Building one instance set per table-start setting")
    ...
    for name, config in CANDIDATES.items():
        config = dict(config)
        ts = config.pop("_table_start"); yrs = config.pop("_years")
        key = (ts, tuple(yrs))
        if key not in built:
            built[key] = build_instances(borough_name, table_start=ts, years=list(yrs))
            logger.info("Built %d instances for table_start=%s years=%s", len(built[key][0]), ts, yrs)
        instances, edge_index = built[key]
        if len(instances) < N_WINDOWS + 2:
            raise ValueError(f"Only {len(instances)} instances for table_start={ts} - need at least {N_WINDOWS + 2}.")
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "tablestart_control_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "tablestart_control_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "tablestart_control_summary.csv")
