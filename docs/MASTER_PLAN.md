...
|---|---|---|---|
| Westminster | **79.75% ± 0.89** (5 seeds) | 68.98% | **better, p<0.0001** |
| Tower Hamlets | **82.75% ± 2.13** (5 seeds) | 72.24% | **better, p=0.0004** |
| Lambeth | **77.75% ± 1.67** (5 seeds) | 76.59% | **TIE, p=0.1941** |
| **POOLED** | **80.08% ± 2.62** (5 seeds) | **72.60%** | **better, p<0.000115** |
...
| V4 | Architecture ablation (7 arms) | ☑ | Each arm differs in exactly 1 key (verified programmatically) | **TOPOLOGY matters, CAPACITY does not**: layers 1->2 -26.46 (p=0.0004); encoder order -13.61 (p=0.0135); decoder -3.50 (null); hidden 42/42 -2.87 (null); weight_decay +2.53 -> **later rejected at n=18 (V9)**. Effects exceed the 19-pt gap -> non-additive |
...
| V7 | Re-verify null ledger under final config | ☑ | Spot-check road class under BOTH configs | **Road class NOT null - significantly HARMFUL** (-6.87 at 13 feat, -4.49 at 35 feat, 0/6 wins, p=0.0127). **Third old null overturned.** Pre-2026-09-04 ledger does NOT transfer and must not be cited as-is |
...
| ID | Task | State | Notes |
|---|---|---|---|
| V11 | Multi-seed all three boroughs | ☑ | MANDATORY before any output | **DONE, then SUPERSEDED 2026-09-06** by the full regeneration (`run_headline_multiseed.py` + `make_headline_table.py`). Superseded figures, kept for the record: WM 80.03±1.40, TH 82.63±2.01, Lam 77.75±1.67, pooled 80.14±1.12. **Current: WM 79.75±0.89 (p<0.0001), TH 82.75±2.13 (p=0.0004), Lam 77.75±1.67 (tie), pooled 80.08±2.62 (p<0.0001).** The pooled ±1.12 was never reproducible - the sample SD is 2.60 and no other convention gives 1.12. Seed-42 bias: +1.69/-1.11/+1.30 - borough-specific, not transferable |
...
| S2b | S2 redone: seed-averaged + PAIRED | ☑ | **R10 compliance for the paper's central claim.** 5 seeds x 6 windows x 3 boroughs, paired t-test + Wilcoxon (`scripts/run_s2_paired_comparison.py`). Pooled n=18: EB **-3.76 (p=0.0116)**, raw count **-3.85 (p=0.0227)**, matched-1825d **-0.90 (p=0.6102, 9/18 wins)**. **SIGN CORRECTED**: the matched-horizon gap was recorded as +0.85 for the GNN; it is -0.90 against. A tie either way. Seed-averaging made the uncapped result SIGNIFICANT where single-seed could produce no p-value at all. Cross-check: pooled GNN 80.13 reproduces V11's 80.14 from a different code path |
...
| S5 | **Extend history ceiling to 9yr** | **☑ SETTLED, 5 seeds x 2 boroughs** | **Pooled −0.12 (p=0.8617, 5/12 windows)**; Lambeth −0.46 (p=0.7015), Westminster +0.21 (p=0.8281). The model gains nothing from history beyond 5 years. **The effect reverses sign BETWEEN SEEDS within each borough** (Lam +0.27/+1.53/−0.72/−1.79/−1.58; WM −1.93/+0.96/+2.60/−0.86/+0.28), so no single-seed run was interpretable — which is why both the original claim (−0.72) and the withdrawal trigger (+2.98) were seed artefacts. The claim is RESTORED with proper evidence: the sort converts the same extra history into +2.95 (p=0.0409) while the model manages −0.12. `scripts/run_s5_multiseed_analysis.py` |
...
| S7 | **Horizon finding on independent US data** | ☑ **PARTIAL** (2026-09-06) | ML4RoadSafety Delaware: 458,282 crashes, 36,466 edges, 166 months (~45× London volume). **At our own top-20% cut it does NOT replicate** — 5.3 pts against London's 61.2, because a 1-month lookback already scores 87.75% and the task is saturated. **With headroom it does**: 18.4 pts at top-5%, 14.5 at top-1%. Direction replicates; the 61-pt magnitude is London-specific. **Mechanism UNRESOLVED** — the obvious candidate (Delaware less tie-saturated) is refuted by measurement: Delaware is MORE tied at 1 month (97.6%) than London at 30d (94.2%). `scripts/run_horizon_us_replication.py` |
...
| ID | Task | State | Notes |
|---|---|---|---|
| P1 | arXiv preprint | ◐ **substantially drafted** | `paper/DRAFT_preprint.md`: abstract, §1-6, Figure 1 (horizon curve), all 16 claims machine-verified against source. **COMPLETE 2026-09-06.** Abstract, §1 motivation, §2 related work, §3 methods, §4 replication result, §5 trivial baseline + horizon curve, §6 measurement properties, §7 reproducibility, §8 limitations, Figure 1. All 16 claims machine-verified against source; cross-document consistency 14/14. Ready for a read-through before submission. Not blocked on Phase 1 - Phase 1 is complete (20 rows ☑). |
