"""MAX-VALUE FEATURE SCALING instead of z-score standardisation -
taken directly from the reference implementation's own code, 2026-09-03.

**Source**: github.com/ZhuangDingyi/STZINB, the public predecessor to
Gao et al.'s STZITD-GNN by the same group (the STZITD repo's own README
points here for data//code, and its London crash dataset is explicitly
"available on request" - i.e. genuinely not reproducible from public
sources). Their `utils.py` scales model INPUTS by the training-set
maximum (`T_inputs = T_inputs / E_maxvalue`) and leaves TARGETS raw,
denormalising predictions afterwards. They never z-score.

**Why this is the strongest remaining hypothesis, and it is measured
rather than argued**: this project has always z-scored, and on real
Lambeth data its z-scored features reach an absolute maximum of ~172
(measured while diagnosing the encoder-order NaN). That is what
z-scoring does to a 99.98%-zero count column - near-zero standard
deviation turns each rare non-zero into an enormous z-score. Values of
that magnitude in GAT attention logits are precisely the condition that
produced attention-parameter NaNs at lr=0.01 and forced a 10x lower
learning rate. Max-scaling maps every feature into [-1, 1] instead.

This also fits the one pattern that has actually held all session:
**every change to the DATA REPRESENTATION has mattered (the network
source, +11.59 points, p=0.0010), while every change to model
architecture or feature breadth has been null.** Feature scaling is a
data-representation change, not a model change.

Identical to `run_ucl_comparison_multiwindow_os_open_roads.py` in every
other respect - same real OS network, same POI+socio features, same
heads=3, same six held-out windows - so the comparison isolates the
scaler alone.
"""
...
logger = logging.getLogger("run_ucl_comparison_maxscale")
...
# source alone.
CANDIDATES = {
    "heads=3 + REAL OS network + MAX-VALUE scaling (reference impl) instead of z-score": dict(
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        mean, std = fit_feature_maxscaler(train_x_seqs)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_maxscale.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_maxscale.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_maxscale.csv")
