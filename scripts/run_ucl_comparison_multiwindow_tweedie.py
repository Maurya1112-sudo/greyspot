"""Multi-window evaluation of the full Zero-Inflated Tweedie (ZITD)
decoder - built 2026-09-01 immediately after implementing
`zero_inflated_tweedie_nll` (grounded directly in the paper's own PDF
text, Eq. 3-6 - see `gat_temporal.py`'s docstring for the full
derivation and its disclosed deviance-based approximation for y > 0).
This is the paper's OWN reported decoder, tested here for the first
time on real data, against the TCR target (`tcr_score` - Gao's exact
severity-weighted target definition, Eq. 7.1) - Tweedie's compound
Poisson-Gamma structure is architecturally appropriate for a severity-
weighted quantity in a way it is not for this project's plain integer
`collision_count` (where ZINB was chosen instead as the honest
distributional upgrade - see that function's own docstring).
...
Run from the project root: python scripts/run_ucl_comparison_multiwindow_tweedie.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_tweedie")
...
# specifically "does the paper's OWN decoder distribution help," not
# "what's the best architecture for this target" (a separate sweep; this
...
# spends its window budget on one config). zero_inflated=True must be
# turned OFF in `defaults` for this candidate (tweedie implies its own
# zero-inflation, and GATTemporal rejects negative_binomial+tweedie
# together, but zero_inflated=True + tweedie=True would just be redundant,
# not an error - excluded here for clarity, not because it would crash).
CANDIDATES = {
    "heads=3, ZITD/Tweedie decoder (Gao et al.'s own reported decoder, rho=1.5)": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_tweedie.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_tweedie.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_tweedie.csv")
