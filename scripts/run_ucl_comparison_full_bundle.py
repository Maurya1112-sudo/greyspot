...
logger = logging.getLogger("run_ucl_comparison_full_bundle")
...
# source alone.
CANDIDATES = {
    # THE PAPER'S FULL HYPERPARAMETER BUNDLE, combined, on our winning
    # encoder order (spatial_first) and the real OS Open Roads network -
    # never tested together. Every ingredient below has been tested in
    # ISOLATION and found null or negative on its own:
    #   - hidden=42 alone: false positive (won Lambeth +1.46, lost
    #     Westminster -9.97, pooled p=0.2986 negative)
    #   - 2-layer alone: only ever tested confounded with temporal_first
    #     encoder order + lr=0.001 (its own separate finding), never at
    #     our winning spatial_first/lr=0.01 configuration
    #   - epochs=20 alone: badly underfits at light-protocol density
    #   - weight_decay=0.01 alone: untested in isolation from the above
    #
    # The hypothesis this closes: hidden=42's Westminster failure may be
    # OVERFITTING from added capacity trained for 200 epochs with ZERO
    # weight decay - a combination the paper never runs. The paper pairs
    # 42 hidden units with wd=0.01 AND early-stopping patience=10 AND
    # only 20 epochs. Isolated-variable testing cannot see this
    # interaction by construction; only the full bundle can.
    "full paper bundle (42/42, 2-layer, wd=0.01, 20ep+ES10)": dict(
        heads=3, gat_layers=2, gat_hidden=42, gru_hidden=42,
        negative_binomial=False, weight_decay=0.01,
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_full_bundle.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_full_bundle.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_full_bundle.csv")
