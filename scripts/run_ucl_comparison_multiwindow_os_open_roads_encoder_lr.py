...
# source alone.
CANDIDATES = {
    # The control run established that each encoder order has its OWN
    # viable learning rate: the default order scores 63.59% at lr=0.01 but
    # collapses to 16.76% at lr=0.001, while the paper's order diverges to
    # NaN at lr=0.01 and reaches 50.85% at lr=0.001. Neither single lr is
    # a fair test of both. lr=0.001 was simply the first STABLE value
    # tried for the paper's order, not its optimum - this sweep looks for
    # that optimum across the stable range, so the architecture comparison
    # is made at each order's own best setting rather than at a value
    # tuned for the other one.
    "PAPER order (GRU->GAT), lr=2e-3": dict(
        heads=3, gat_layers=1, negative_binomial=False, encoder_order="temporal_first", lr=0.002,
    ),
    "PAPER order (GRU->GAT), lr=3e-3": dict(
        heads=3, gat_layers=1, negative_binomial=False, encoder_order="temporal_first", lr=0.003,
    ),
    "PAPER order (GRU->GAT), lr=5e-3": dict(
        heads=3, gat_layers=1, negative_binomial=False, encoder_order="temporal_first", lr=0.005,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_encoder_lr.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_encoder_lr.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_encoder_lr.csv")
