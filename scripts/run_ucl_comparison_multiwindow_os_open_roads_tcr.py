"""TCR target (the paper's own severity-weighted crash score, thesis
Definition 1: fatal*3 + serious*2 + slight*1) + the REAL OS Open Roads
network, combined for the first time - built 2026-09-02 to chase
Lambeth's one remaining open gap (63.59% vs the paper's 76.59%).

Both ingredients were tested before, never together: TCR-vs-plain-count
was tested standalone on the OSMnx network (statistically
indistinguishable, 50.17% vs 49.57%) long before the real network was
confirmed as this project's biggest lever; the real network has so far
only ever been tested with the plain-count target. Since TCR is the
paper's actual target, re-checking it now that the network matches
their own data source is a genuinely new, untried combination, not a
repeat of the earlier negative.

Run on Lambeth specifically (not Westminster) - same reasoning as the
real-network heads-sweep: Lambeth is the one open gap, so this is
where a genuine improvement would matter most.

Otherwise identical to `run_ucl_comparison_multiwindow_os_open_roads.py`
(real network via `build_borough_graph_os_open_roads`, POI+socio
features, heads=3, light protocol) - only the target column changes
(`tcr_score` instead of the default `collision_count`).

Run from the project root: python scripts/run_ucl_comparison_multiwindow_os_open_roads_tcr.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_tcr")
...
CANDIDATES = {
    "heads=3 + POI + socio-demographic + REAL OS Open Roads network + TCR target": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_tcr.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_tcr.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_tcr.csv")
