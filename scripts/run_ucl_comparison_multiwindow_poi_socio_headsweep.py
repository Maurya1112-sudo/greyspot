"""Architecture sweep ON TOP of POI + socio-demographic features - the
one combination explicitly flagged as untried in
`docs/decision_log.md`/project memory ("a formal architecture sweep ON
TOP of these features (not yet tried - this pass only tested them with
the existing heads=3 config)"). Both `heads=3` (paired t-test p=0.036
vs heads=1) and POI+socio-demographic features (p=0.0094 across 3
boroughs) are independently confirmed improvements over their own
respective baselines - this asks whether the OPTIMAL head count changes
once the richer feature set is already present, rather than assuming
heads=3 (tuned without these features) is still the best choice.

Copied from `run_ucl_comparison_multiwindow_poi_socio.py` (identical
`build_instances` - same POI/socio-demographic feature set, same raw
OSMnx network, no consolidation) - only `CANDIDATES` changes, to a
heads=1/2/3/4 sweep at gat_layers=1 (2-layer GNNs were already tested,
without these features, and found unhelpful - see project memory - not
re-swept here to keep this run focused on the one untested axis).

Per the project's "test one borough first, only expand if promising"
policy (2026-09-02), this runs Westminster only; a promising result
would be cross-checked on Lambeth/Tower Hamlets same as every other
confirmed finding this session.

Run from the project root: python scripts/run_ucl_comparison_multiwindow_poi_socio_headsweep.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_poi_socio_headsweep")
...
# Heads=1..4 at gat_layers=1, all with POI + socio-demographic features
# already present - the untested axis this script exists to answer.
CANDIDATES = {
    "heads=1 + POI + socio-demographic": dict(heads=1, gat_layers=1, negative_binomial=False),
    "heads=2 + POI + socio-demographic": dict(heads=2, gat_layers=1, negative_binomial=False),
    "heads=3 + POI + socio-demographic": dict(heads=3, gat_layers=1, negative_binomial=False),
    "heads=4 + POI + socio-demographic": dict(heads=4, gat_layers=1, negative_binomial=False),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_poi_socio_headsweep.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_poi_socio_headsweep.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_poi_socio_headsweep.csv")
