...
CANDIDATES = {
    ...
    # 2026-09-01: the earlier combined test (heads=3 + hidden=42 + lr=1e-3 +
    # weight_decay=1e-4, mean 20.55%) confounded THREE changes at once and
    # a much lower lr with unchanged epochs=200 is a plausible confound
    # (see docs/decision_log.md) - this isolates weight_decay alone,
    # keeping the already-proven lr=0.01 and gat_hidden=16 unchanged, to
    # find out whether weight_decay itself helps or hurts independent of
    # that confound.
    "heads=3 + weight_decay=1e-4 (isolated)": dict(heads=3, gat_layers=1, negative_binomial=False, weight_decay=1e-4),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_wd.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_wd.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_wd.csv")
