...
logger = logging.getLogger("run_pruning_v2")
...
# source alone.
# PRUNING TEST v2, built 2026-09-04.
#
# **Why a v2.** The original pruning test (2026-09-03) was INVALIDATED by
# the config-rebinding bug: both arms trained on identical features for
# 5 of the 6 windows, so it never tested anything. It also compared
# against the superseded 30-feature configuration rather than the final
# 35-feature one.
#
# **The question.** The 5 casualty-breakdown columns are a decomposition
# of `collision_count` itself - measured correlations 0.14-0.85 with it,
# and each is 2-50x SPARSER (n_fatal is non-zero on 0.00043% of
# segment-days). They may fragment an already-rare signal into noisier
# collinear pieces. Removing them is the one feature change this project
# has never cleanly tested.
#
# Uses `window_config` (the bug fix), so the subset genuinely applies to
# every window - verified by asserting the feature count per window.
_CASUALTY = ["n_fatal_casualties", "n_serious_casualties", "n_slight_casualties",
             "n_pedestrian_casualties", "n_cyclist_casualties"]
_PRUNED = [c for c in FEATURE_COLUMNS if c not in _CASUALTY]

CANDIDATES = {
    "FINAL config (35 features)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=list(FEATURE_COLUMNS),
    ),
    "pruned (30 features, casualty breakdown dropped)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=_PRUNED,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "pruning_v2_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "pruning_v2_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "pruning_v2_summary.csv")
