...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_junction_home")

# Keep most of each junction crash on its snapped segment, spreading
# only the remainder - full (even) redistribution tested significantly
# WORSE (p=0.0432) with prediction variance collapsing to std 2.66%
# (vs 11.26%), i.e. it flattened exactly the sharp distinctions a
# top-20% ranking needs. 0.7 is a deliberate middle ground, chosen
# once and not tuned against the test windows.
HOME_SHARE = 0.7
...
# source alone.
CANDIDATES = {
    "heads=3 + POI + socio-demographic + REAL OS network + PARTIAL junction redistribution (home_share=0.7)": dict(
...
def build_instances(borough_name: str):
    ...
    redistributed = redistribute_junction_crashes(snapped, graph, home_share=HOME_SHARE)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_junction_home.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_junction_home.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_junction_home.csv")
