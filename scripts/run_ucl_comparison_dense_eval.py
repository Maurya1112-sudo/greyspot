...
logger = logging.getLogger("run_ucl_comparison_dense_eval")
...
# DENSE EVALUATION GRID, added 2026-09-03. Purpose: reduce the WIDTH of
# the confidence interval around this project's score, so the comparison
# against Gao et al.'s published figures becomes decisive rather than
# inconclusive - WITHOUT changing the model at all.
#
# The problem: at stride=90 there are only 6 evaluation windows per
# borough. With a per-window std of ~5.6 points that gives a 95% CI of
# roughly +/-5.9 points, which comfortably contains the paper's figure -
# so even a 3-point lead reads as a statistical tie. The estimate is
# imprecise, not necessarily wrong.
#
# The fix is more measurements of the same thing: stride=14 over the same
# calendar span yields ~38 windows, narrowing the CI to roughly +/-1.8
# points by the sqrt(n) rule.
#
# **Why stride=14 and not lower.** HORIZON is 14 days, so stride=14 makes
# consecutive windows' TARGET periods exactly adjacent and
# NON-OVERLAPPING. A shorter stride would reuse the same crash days in
# multiple windows, making them correlated - which narrows the interval
# without adding information, i.e. manufactures precision rather than
# earning it. That would be a way of faking significance and is
# deliberately not done.
#
# This run is NOT paired-comparable with the six-window results (the
# held-out dates differ), and is not used to re-test the multi-year
# improvement - that is already established at n=6 (p=0.0092) and n=18.
# Its only job is a tighter interval on the final model's score.
...
N_WINDOWS = 38  # dense grid: ~532 days of non-overlapping target periods, matching the 6x90d span
FEATURE_COLUMNS = [
...
# source alone.
CANDIDATES = {
    "final model (multi-year history), DENSE 38-window evaluation": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_denseeval.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_denseeval.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_denseeval.csv")
