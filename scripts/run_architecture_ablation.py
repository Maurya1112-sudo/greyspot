...
logger = logging.getLogger("run_architecture_ablation")
...
# source alone.
# ARCHITECTURE ABLATION, added 2026-09-04.
#
# **Why.** The head-to-head found a ~19-point gap between this project's
# architecture (78.94%) and a reimplementation of Gao et al.'s (59.93%)
# on identical data and windows. But those two configurations differ in
# FIVE ways at once - encoder order, layer count, hidden size, decoder
# family, weight decay - so "architecture is worth 19 points" is exactly
# the same multiple-changes-single-attribution error that produced this
# session's false "+15.85 from crash history" claim (see
# docs/publication_plan.md section 0).
#
# Each arm below starts from THIS PROJECT'S configuration and changes
# EXACTLY ONE thing. The differences should roughly sum to the 19-point
# gap; if they do not, there is an interaction worth reporting.
#
# **Learning rate is a confound, not a free choice.** The encoder-order
# change demonstrably needs its own lr (spatial_first diverges to NaN
# below ~1e-2 is false - rather temporal_first diverges AT 1e-2, and its
# own optimum on this feature set was swept to 5e-4). So the
# encoder-order arm is run at BOTH this project's lr and the swept
# optimum for that order, and the better of the two is what the
# comparison uses. Penalising an architecture for a learning rate tuned
# for a different one would be a straw man.
_BASE = dict(heads=3, gat_layers=1, gat_hidden=16, gru_hidden=32,
             negative_binomial=False, lr=0.01)

CANDIDATES = {
    "BASELINE (this project's architecture)": dict(_BASE),
    "change ONLY encoder order -> temporal_first (their order), lr=0.01": dict(_BASE, encoder_order="temporal_first"),
    "change ONLY encoder order -> temporal_first, at its own lr=5e-4": dict(_BASE, encoder_order="temporal_first", lr=5e-4),
    "change ONLY layers 1 -> 2 (their depth)": dict(_BASE, gat_layers=2),
    "change ONLY hidden 16/32 -> 42/42 (their size)": dict(_BASE, gat_hidden=42, gru_hidden=42),
    "change ONLY decoder ZIP -> Zero-Inflated Tweedie (theirs)": dict(_BASE, tweedie=True, negative_binomial=False),
    "change ONLY weight decay 0.0 -> 0.01 (theirs)": dict(_BASE, weight_decay=0.01),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "architecture_ablation_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "architecture_ablation_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "architecture_ablation_summary.csv")
