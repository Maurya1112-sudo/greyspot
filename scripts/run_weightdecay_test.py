...
logger = logging.getLogger("run_weightdecay_test")
...
# source alone.
# WEIGHT-DECAY TEST, built 2026-09-04.
#
# The architecture ablation found that Gao et al.'s weight_decay=0.01 -
# a setting this project has always left at 0.0 - IMPROVES Lambeth by
# +2.53 points (79.44% -> 81.97%, p=0.0800). It is the only arm of that
# ablation that beat the baseline.
#
# p=0.0800 at n=6 is precisely the profile that has twice produced false
# positives in this project (hidden=42/42 and the architecture ensemble
# both looked good on one borough and died on the third). So this is NOT
# adopted on the Lambeth result; it goes to cross-borough replication
# first, exactly as the standing policy requires.
#
# It is also a mildly interesting finding either way: regularisation
# helping here is consistent with the overfitting concern that motivates
# the V5 audit - 35 features fitted from 6-11 temporal instances.
CANDIDATES = {
    "final config, weight_decay=0.0 (current)": dict(
        heads=3, gat_layers=1, negative_binomial=False, weight_decay=0.0,
    ),
    "final config, weight_decay=0.01 (theirs)": dict(
        heads=3, gat_layers=1, negative_binomial=False, weight_decay=0.01,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "weightdecay_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "weightdecay_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "weightdecay_summary.csv")
