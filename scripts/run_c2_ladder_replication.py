...
logger = logging.getLogger("run_c2_ladder_replication")
...
# source alone.
# FEATURE-COUNT LADDER (V10), built 2026-09-04.
#
# The V5 overfitting audit's single-window probe hinted that 26 features
# scored better than 35 (78.21% vs 73.72%). One window proves nothing, so
# this runs the full 6-window protocol with paired significance testing.
#
# **A prediction, stated before running.** V6 found that dropping 5
# casualty columns costs -6.89 points (p=0.0014, 0/6 windows), and V5's
# label-shuffle passed cleanly. Both indicate the model is UNDERFIT at 35
# features, not overfit. So the single-window hint is most likely noise,
# and the ladder should show more features helping or being flat - not a
# peak at 26. If 26 genuinely beats 35 across 6 windows, that contradicts
# V6 and needs explaining rather than adopting.
#
# Ladder groups are dropped in a FIXED a-priori order (least-justified
# first), decided before seeing any result, so this is not selection on
# the evaluation set.
_HISTORY = [c for c in FEATURE_COLUMNS if c.startswith("collision_count")]
_GEOM = ["length", "u_degree", "v_degree", "day_of_week"]
_CASUALTY = [c for c in FEATURE_COLUMNS if c.startswith("n_")]
_EXPOSURE = [c for c in FEATURE_COLUMNS if c.startswith("aadf_") or c == "has_aadf"]
_POI = [c for c in FEATURE_COLUMNS if c.startswith("poi_") or c == "has_poi"]
_SOCIO = [c for c in FEATURE_COLUMNS if c.startswith("imd_") or c in ("population_density", "has_socio_demographic")]

_L12 = _GEOM + _HISTORY                      # 12: geometry + all crash history
_L17 = _L12 + _CASUALTY                      # 17: + casualty breakdown
_L20 = _L17 + _EXPOSURE                      # 20: + traffic exposure
_L25 = _L20 + _POI                           # 25: + POI
_L35 = list(FEATURE_COLUMNS)                 # 35: + socio-demographic (final)

CANDIDATES = {
    "12 features (geometry + crash history only)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L12),
    "17 features (+ casualty breakdown)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L17),
    "20 features (+ traffic exposure)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L20),
    "25 features (+ POI)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L25),
    "35 features (+ socio-demographic = FINAL)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L35),
}
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        if wanted_cols is not None:
            ...
            # Guard against the exact bug that invalidated v1: assert the
            # subset actually applied on EVERY window, not just the first.
            assert train_slice[0][0].shape[-1] == len(wanted_cols), (
                f"feature subsetting failed on window {held_out_idx}: "
                f"got {train_slice[0][0].shape[-1]}, expected {len(wanted_cols)}")
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "c2_ladder_repl_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "c2_ladder_repl_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "c2_ladder_repl_summary.csv")
