...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_roadclass")
...
FEATURE_COLUMNS = [
    ...
    *SOCIO_DEMOGRAPHIC_COLUMNS, "has_socio_demographic",
    # NEW (2026-09-02): the paper's own "Roads | 8 classes" and
    # "Date | 4 classes" inputs - see docs/gap_closing_plan.md.
    *ROAD_CLASS_COLUMNS,
    *DATE_FEATURE_COLUMNS,
]
...
# source alone.
CANDIDATES = {
    "heads=3 + POI + socio + REAL OS network + ROAD CLASS (8) + DATE (4) features": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_roadclass.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_roadclass.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_roadclass.csv")
