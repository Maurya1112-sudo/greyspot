...
from greyspot.ingest.weather import WEATHER_DAILY_VARS, load_london_daily_weather  # noqa: E402
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_weather")
...
# 2026-09-01: added the paper's own "Meteorological Characteristics"
# feature category (Gao et al.'s PhD thesis, Table 7.2) - this project's
# daily pipeline had zero weather signal before now. Source:
# ingest/weather.py (Open-Meteo, free, no key) - see that module's
# docstring for why this substitutes for the paper's own Met Office
# source without changing what's actually measured.
FEATURE_COLUMNS = [
    ...
    *WEATHER_DAILY_VARS, "has_weather",
]

RAW_DIR = ROOT / "data" / "raw"
INTERIM_DIR = ROOT / "data" / "interim"
WEATHER_PATH = RAW_DIR / "weather" / "london_daily_weather_2021_2025.json"
...
CANDIDATES = {
    # 2026-09-01: heads=3 is the best architecture found so far (49.57%
    # mean, docs/decision_log.md). This run adds real weather data (the
    # paper's own feature category, previously entirely absent from this
    # pipeline - see ingest/weather.py) on top of it, to test whether a
    # genuinely new, paper-matched data source moves the number further,
    # independent of every architecture/target/training-loop lever
    # already tried and closed out today.
    "heads=3 + weather (Met Office-derived, via Open-Meteo)": dict(heads=3, gat_layers=1, negative_binomial=False),
}
...
def build_instances(borough_name: str):
    ...
    weather = load_london_daily_weather(WEATHER_PATH)
    table = attach_daily_weather_features(table, weather)
    logger.info("Weather attached: %d/%d rows have a real reading (has_weather=1)", table["has_weather"].sum(), len(table))
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_weather.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_weather.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_weather.csv")
