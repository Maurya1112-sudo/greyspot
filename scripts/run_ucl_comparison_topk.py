...
logger = logging.getLogger("run_ucl_comparison_topk")
...
# source alone.
CANDIDATES = {
    # TOP-K HINGE auxiliary loss (see models.gat_temporal.top_k_hinge_loss).
    # The model is trained on a distributional NLL but EVALUATED on a
    # ranking metric - a mismatch the learning-to-rank literature
    # addresses with metric-aware losses. An earlier plain pairwise hinge
    # hurt (48.29% / 40.76% vs a ~50% baseline) because it weights all
    # ~23,000 daily (positive, negative) pairs equally, when AccHR@20
    # depends only on whether positives clear the top-20% boundary.
    # This loss penalises exactly that shortfall. Three weights, spanning
    # "gentle nudge" to "co-equal with the NLL", fixed in advance.
    "top-k hinge, weight=0.1": dict(
        heads=3, gat_layers=1, negative_binomial=False, topk_loss_weight=0.1,
    ),
    "top-k hinge, weight=0.5": dict(
        heads=3, gat_layers=1, negative_binomial=False, topk_loss_weight=0.5,
    ),
    "top-k hinge, weight=1.0": dict(
        heads=3, gat_layers=1, negative_binomial=False, topk_loss_weight=1.0,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_topk.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_topk.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_topk.csv")
