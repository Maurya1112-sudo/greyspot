...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_junction")
...
# source alone.
CANDIDATES = {
    "heads=3 + POI + socio-demographic + REAL OS network + JUNCTION redistribution": dict(
...
def build_instances(borough_name: str):
    ...
    redistributed = redistribute_junction_crashes(snapped, graph)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_junction.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_junction.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_junction.csv")
