...
CANDIDATES = {
    ...
    # 2026-09-01: does heads=3 (best config, 49.57% at 200 epochs) still
    # have headroom left in it, or has full-batch GD already converged by
    # epoch 200? No early-stopping/validation-loss tracking exists in this
    # pipeline (a real, disclosed gap - see docs/decision_log.md), so this
    # tests one concrete hypothesis (more epochs helps) directly via the
    # only lever available without building early-stopping machinery.
    "heads=3, epochs=500 (vs 200)": dict(heads=3, gat_layers=1, negative_binomial=False, epochs=500),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_epochs500.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_epochs500.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_epochs500.csv")
