...
logger = logging.getLogger("run_headtohead_stzitd")
...
# source alone.
# CONTROLLED HEAD-TO-HEAD vs Gao et al.'s architecture, added 2026-09-04.
#
# **The gap this closes.** Every comparison in this project so far has
# been this project's numbers against the paper's PUBLISHED numbers -
# different years (2022-24 vs their 2019), different protocol
# (walk-forward vs their within-year 6:2:2), and a target measurably
# sparser than theirs. A referee can attribute any difference to
# protocol rather than method, and would be right to.
#
# This runs THEIR architecture on THIS project's exact data, graph and
# evaluation windows, so the only thing varying is the model. Their
# spec, from thesis Section 7.4.1 and 7.2.3: Zero-Inflated Tweedie
# decoder, GRU->GAT encoder order ("historical temporal embeddings ZT
# ... are subsequently directed to graph neural encoders"), two GNN
# layers, hidden 42, 3 attention heads, Adam, weight decay 0.01.
#
# **Two deliberate deviations, both disclosed:**
# 1. lr=5e-4 rather than their published 0.01. Their encoder order
#    diverges to NaN at 0.01 on this data (documented 2026-09-02). A
#    dedicated six-point sweep on the CURRENT 35-feature set found 5e-4
#    to be its optimum, and it is a genuine local peak:
#      2e-3 44.88% | 5e-4 50.24% | 1e-4 45.68%
#      3e-5 26.61% | 1e-5 24.74% | 3e-6 22.47%
#    Notably their own predecessor code (STZINB) uses lr=1e-5, which is
#    far too low here (24.74%) - so simply copying their published
#    hyperparameters would ALSO have understated their architecture.
#    An earlier attempt at this comparison used 2e-3 and scored their
#    model ~5 points below its own optimum; that run was discarded
#    rather than reported. Grid is coarse (half-decade steps), so 5e-4
#    is a good setting, not provably the global best.
# 2. epochs=200 without early stopping rather than their 20 with
#    patience=10. Their budget assumes far more training instances than
#    this walk-forward protocol provides; at 6-11 instances it collapses
#    to a near-constant prediction (measured: 19.54%/10.54%). Again,
#    the fair test is their architecture at a budget that lets it train.
#
# **The key contrast is the last two rows**: their architecture WITH and
# WITHOUT the long history. If long history helps THEIR model too, the
# finding is a property of the problem rather than of this project's
# architecture - which is a stronger and far more publishable claim, and
# it simultaneously removes the protocol objection.
_SHORT_HISTORY = ["collision_count_7d", "collision_count_14d", "collision_count_30d"]
_LONG_HISTORY = ["collision_count_90d", "collision_count_365d", "collision_count_730d",
                 "collision_count_1095d", "collision_count_1825d"]
_SHORT_ONLY = [c for c in FEATURE_COLUMNS if c not in _LONG_HISTORY]

CANDIDATES = {
    "Gao et al. architecture (ZI-Tweedie, GRU->GAT, 2-layer/42), SHORT history": dict(
        heads=3, gat_layers=2, gat_hidden=42, gru_hidden=42,
        tweedie=True, encoder_order="temporal_first", weight_decay=0.01,
        lr=5e-4, feature_columns=_SHORT_ONLY,
    ),
    "Gao et al. architecture, LONG history (the key test)": dict(
        heads=3, gat_layers=2, gat_hidden=42, gru_hidden=42,
        tweedie=True, encoder_order="temporal_first", weight_decay=0.01,
        lr=5e-4, feature_columns=list(FEATURE_COLUMNS),
    ),
    "This project's architecture, SHORT history": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_SHORT_ONLY,
    ),
    "This project's architecture, LONG history (final model)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=list(FEATURE_COLUMNS),
    ),
}
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        # NOTE: bind to a NEW name. Writing `config = dict(config)` here
        # rebinds the loop-outer variable, so on the SECOND iteration the
        # dict is already popped, `wanted_cols` becomes None, and every
        # window after the first silently trains on the FULL feature set.
        # That bug invalidated several runs on 2026-09-04 before it was
        # found by mechanical diffing (see docs/decision_log.md).
        window_config = dict(config)
        wanted_cols = window_config.pop("feature_columns", None)
        if wanted_cols is not None:
            col_idx = [FEATURE_COLUMNS.index(c) for c in wanted_cols]
            train_slice = [(x[:, :, col_idx], y, st) for x, y, st in train_slice]
            held_out_x_raw = held_out_x_raw[:, :, col_idx]
        ...
        model = train_gat_temporal_walkforward(
            train_instances, edge_index, **{**defaults, **window_config},
        )
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "headtohead_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "headtohead_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "headtohead_summary.csv")
