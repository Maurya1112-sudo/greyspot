...
CANDIDATES = {
    # 2026-09-01, fourth sweep: reading the full PhD thesis (Gao, UCL,
    # discovery.ucl.ac.uk/id/eprint/10210801) directly - not just the
    # journal paper's OCR'd text - preserved the subscript the earlier
    # extraction mangled: "N_epoch=20" (confirmed - N is defined elsewhere
    # in the same document as the road COUNT, in the thousands, so "N=20"
    # was never a coherent input-window reading). Also confirmed
    # unambiguously: "The GNNs in STZITD-GNNs... are all two-layered. The
    # hidden units are set to 42" with lr=0.01, weight_decay=0.01 (the
    # SAME values already in this pipeline's defaults - the earlier
    # "corrected lr/wd" test that scored 20.55% used lr=1e-3/wd=1e-4 from
    # a DIFFERENT model's config in a sibling repo, a real confound now
    # resolved). This candidate is the actual, fully paper-matched
    # configuration - 2-layer, hidden=42, heads=3, lr=0.01, wd=0.01,
    # epochs=20 (not 200) - tested properly for the first time.
    "FAITHFUL: heads=3, 2-layer, hidden=42, lr=0.01, wd=0.01, epochs=20": dict(
        heads=3, gat_layers=2, gat_hidden=42, gru_hidden=42,
        weight_decay=0.01, epochs=20, negative_binomial=False,
    ),
    # Same, but epochs=200 (this pipeline's own usual budget) in case
    # epochs=20 alone is too little for THIS project's different feature
    # set/data (16 features here vs whatever the paper's own inputs are) -
    # isolates epoch count from every other now-confirmed paper value.
    "FAITHFUL, epochs=200 (isolate epoch count)": dict(
        heads=3, gat_layers=2, gat_hidden=42, gru_hidden=42,
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_faithful.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_faithful.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_faithful.csv")
