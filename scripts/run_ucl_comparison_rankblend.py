"""RANK BLENDING: GNN + historical crash rate - a genuinely different
method, not another hyperparameter, 2026-09-03.

**The reasoning.** AccHR@20 is a pure RANKING metric: it only asks
whether roads with real crashes land in the model's top 20%. Everything
tried so far optimised a single model's likelihood and then ranked its
output. But the paper's own Table 7.3 reports a HISTORICAL AVERAGE
baseline (no neural network at all) scoring 47.99% on Lambeth, where
this project's GNN scores 63.59%. Two predictors that rank roads
DIFFERENTLY can be combined to beat either alone - this is the standard
result for ranking problems, and it has never been tried here.

**Why it is not double-counting**: the GNN already receives
`collision_count` and rolling 7/14/30-day counts as INPUT features, so
in principle it could learn the historical signal itself. In practice
neural models routinely under-weight a simple strong predictor when it
is one of ~40 inputs, and a likelihood loss optimises calibration
rather than rank order. An explicit rank blend forces the historical
signal back to full strength at ranking time.

**Method**: convert both predictors to per-day RANKS (not raw scores -
their scales are entirely different and a raw average would be
dominated by whichever has larger numbers), then average.
`blend = alpha * rank(model) + (1 - alpha) * rank(history)`.

**Leakage control**: the historical rate for a held-out window is
computed ONLY from that window's own training instances - never from
the evaluated window or anything after it - matching the walk-forward
discipline used everywhere in this project.

**Avoiding tuning-on-test**: alpha=0.5 (plain equal-weight rank
averaging, parameter-free) is the PRE-REGISTERED headline choice. The
full alpha curve is reported alongside it for transparency, but the
0.5 result is the one that counts as a claim - selecting the best alpha
by looking at the evaluation windows would be exactly the kind of
metric-gaming this project has refused throughout.
"""
...
logger = logging.getLogger("run_ucl_comparison_rankblend")
...
# source alone.
CANDIDATES = {
    "RANK BLEND: heads=3 + REAL OS network, GNN x historical-rate rank average": dict(
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        # --- HISTORICAL-RATE PREDICTOR (leakage-safe) ---------------
        # Mean crash count per segment across the TRAINING instances
        # only. `y` in each training instance is [N, horizon]; averaging
        # over instances and horizon gives one historical rate per
        # segment. Never touches the held-out window or anything after.
        # y is [horizon, N] - average over the HORIZON axis (0) to get one
        # historical rate per SEGMENT. (axis=1 would average over segments
        # and yield [horizon], which broadcast-fails downstream.)
        hist = np.mean([y.mean(axis=0) for _, y, _ in train_slice], axis=0)  # [N]
        assert hist.shape[0] == y_pred.shape[1], (
            f"historical rate must be one value per segment: got {hist.shape} vs y_pred {y_pred.shape}"
        )

        def _per_day_ranks(scores_2d):
            """Rank segments within each day (higher score -> higher rank).
            Ranking per day matters: AccHR@20 itself selects the top 20%
            within each day, so a global ranking would mix days that have
            very different overall risk levels."""
            order = np.argsort(np.argsort(scores_2d, axis=1), axis=1)
            return order / max(scores_2d.shape[1] - 1, 1)

        model_ranks = _per_day_ranks(y_pred)                       # [horizon, N]
        hist_ranks = _per_day_ranks(np.tile(hist, (y_pred.shape[0], 1)))

        for alpha in (1.0, 0.75, 0.5, 0.25, 0.0):
            blended = alpha * model_ranks + (1.0 - alpha) * hist_ranks
            blend_metrics = ucl_metric_suite(y_true, blended, lower, upper, top_fraction=0.20)
            tag = "model_only" if alpha == 1.0 else ("history_only" if alpha == 0.0 else f"alpha_{alpha}")
            logger.info("    blend %s: AccHR@20=%.4f", tag, blend_metrics["AccHR"])
            if alpha == 0.5:
                headline_blend = blend_metrics["AccHR"]
            if alpha == 1.0:
                model_only_acc = blend_metrics["AccHR"]
            if alpha == 0.0:
                history_only_acc = blend_metrics["AccHR"]

        metrics = ucl_metric_suite(y_true, y_pred, lower, upper, top_fraction=0.20)
        metrics["AccHR_blend_alpha0.5"] = headline_blend
        metrics["AccHR_model_only_ranked"] = model_only_acc
        metrics["AccHR_history_only"] = history_only_acc
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_rankblend.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_rankblend.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_rankblend.csv")
