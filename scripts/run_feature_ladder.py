...
logger = logging.getLogger("run_feature_ladder")
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "feature_ladder_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "feature_ladder_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "feature_ladder_summary.csv")
