...
logger = logging.getLogger("run_c7_history_replication")
...
# source alone.
# C7 - REPLICATE THE HISTORY-HORIZON CLAIM ON A SECOND BOROUGH.
#
# **Why this is the priority.** +15.87 points is the LARGEST claimed
# effect in this project, and it rests on LAMBETH ALONE. The
# single-borough scoreboard is now 1 replicated out of 6 attempts
# (docs/MASTER_PLAN.md 2b) - four outright failures, one downgrade.
# An unreplicated 15-point claim is the least defensible thing in the
# project.
#
# Isolates history horizon with everything else held fixed: SHORT
# history (7/14/30d, the original 30-day cap) versus the FULL ladder
# (adding 90/365/730/1095/1825d). Same table, same target, same model.
...
CANDIDATES = {
    "SHORT history only (7/14/30d - the original cap)": dict(
    ...
    "FULL history ladder (+90/365/730/1095/1825d)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=list(FEATURE_COLUMNS), _table_start="2021-01-01", _years=[2021, 2022, 2023, 2024]),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "c7_history_repl_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "c7_history_repl_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "c7_history_repl_summary.csv")
