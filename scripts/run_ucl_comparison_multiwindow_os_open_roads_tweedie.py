"""The paper's own Zero-Inflated Tweedie decoder + TCR target + the
REAL OS Open Roads network, combined for the first time - built
2026-09-02 after re-reading the thesis's own text on WHY Lambeth is
their best-performing borough despite being their hardest one.

The thesis states outright: Lambeth has the HIGHEST zero-inflation
rate of the three boroughs (96.71% vs Westminster's 95.72% and Tower
Hamlets' 96.28%) - "Notably, in Lambeth, the borough exhibiting the
highest zero-inflation and data sparsity, the STZITD-GNN model attains
the lowest MAE... it achieved 76.59% accuracy in identifying actual
crashes in the Lambeth case" and "In extreme zero-inflation scenarios,
such as in Lambeth, both the STTD and STZINB models perform poorly...
compared to the STZITD-GNN model." Verified this project's own real-
network data shows the SAME ordering (Lambeth's zero-rate 99.98% vs
Westminster/Tower Hamlets' 99.97%) - a real, corroborated pattern, not
assumed. The paper's own text directly attributes their strong Lambeth
result to the Zero-Inflated Tweedie decoder's handling of extreme
sparsity specifically - a decoder choice this project has tested
before (found WORSE than ZIP, twice, on 2 boroughs) but NEVER combined
with the real network, only with OSMnx.

Run on Lambeth - the one open gap, and the specific borough the
paper's own text says this decoder choice matters most for.

Otherwise identical to `run_ucl_comparison_multiwindow_os_open_roads_tcr.py`
(real network, POI+socio features, heads=3, TCR target, light
protocol) - only the decoder changes: `tweedie=True, zero_inflated=False`
(Tweedie's own zero-gate replaces the ZIP zero-gate, matching this
project's existing Tweedie-testing convention).

Run from the project root: python scripts/run_ucl_comparison_multiwindow_os_open_roads_tweedie.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_tweedie")
...
# real network), unchanged - this run answers ONE question ("does the
# paper's own TCR target change AccHR@20 now that the network matches
# theirs too"), isolated to the target-column variable alone.
CANDIDATES = {
    "heads=3 + POI + socio-demographic + REAL OS Open Roads network + TCR target + ZITD/Tweedie decoder": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_tweedie.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_tweedie.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_tweedie.csv")
