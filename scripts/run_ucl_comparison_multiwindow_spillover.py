"""Multi-window evaluation on the TCR target WITH spillover - built
2026-09-01, the last of this session's "real remaining candidates"
(docs/decision_log.md): Gao et al.'s predecessor paper (STZINB-GNN,
arXiv:2307.13816) states their target incorporates "spillover effects
on first and second-order neighbouring roads" on top of the severity
weighting (`tcr_score`, reconstructed in the sibling
`run_ucl_comparison_multiwindow_tcr.py` script - 50.17%, statistically
indistinguishable from the plain-count baseline). This script tests
whether ALSO adding spillover - `attach_spillover_target`, applied on
top of `tcr_score` (the paper's real target is severity-weighted AND
spillover-diffused, not one or the other) - explains the persistent
AccHR@20 gap where the target-definition fix alone did not.

**Disclosed, not hidden**: neither paper states the exact spillover
decay function or aggregation rule (see `attach_spillover_target`'s own
docstring, `daily_features.py`) - the weights used here (0.5 first-order,
0.25 second-order, reusing the GAT's own line-graph adjacency) are an
explicit, defensible choice for experimentation, not a claimed
replication of Gao's precise formula. This changes the TARGET only, not
the input features (`tcr_score`, `collision_count`, and every severity/
exposure column remain available as INPUTS, matching every other sweep
in this family).
...
Run from the project root: python scripts/run_ucl_comparison_multiwindow_spillover.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_spillover")
TARGET_COL = "tcr_score_spillover"
SPILLOVER_SOURCE_COL = "tcr_score"
SPILLOVER_FIRST_ORDER_WEIGHT = 0.5
SPILLOVER_SECOND_ORDER_WEIGHT = 0.25
...
# specifically "does adding spillover on top of the TCR target matter,"
# not "what's the best architecture for this target" (a separate sweep;
# this project's own architecture-sensitivity finding says a single
# window can't distinguish architectures reliably anyway - this run
# already spends its window budget on one config).
CANDIDATES = {
    "heads=3, TCR+spillover target (disclosed 0.5/0.25 decay, not Gao's exact formula)": dict(
...
def build_instances(borough_name: str):
    ...
    if SPILLOVER_SOURCE_COL not in table.columns:
        raise ValueError(
            f"'{SPILLOVER_SOURCE_COL}' column missing from the segment-day table - "
    ...
    table = attach_spillover_target(
        table, edge_index, segment_order, source_col=SPILLOVER_SOURCE_COL,
        first_order_weight=SPILLOVER_FIRST_ORDER_WEIGHT, second_order_weight=SPILLOVER_SECOND_ORDER_WEIGHT,
    )
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_spillover.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_spillover.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_spillover.csv")
