...
logger = logging.getLogger("run_s6_rank_scaling")
...
# source alone.
# S6 - SPARSE-ROBUST FEATURE SCALING (2026-09-05).
#
# Five independent experiments have now shown that adding sparse count
# columns to this model DEGRADES it: road class (-6.87/-4.49), traffic
# exposure (-1.62/-1.64), casualty breakdown (-1.00), pruning-to-30
# (-6.89) and deep history (-0.72, variance 6.12%->9.83%). The GNN also
# fails to exploit history that a parameter-free sort converts into
# +2.95 (S2/S5).
#
# **The hypothesis**: z-scoring 99.97%-zero count columns produces
# extreme standardised values (absmax ~1179 measured on real data), and
# each added sparse column contributes more extremes that dilute the
# representation of the columns carrying signal.
#
# **The test**: replace z-scoring with a per-feature RANK transform.
# AccHR@20 is a within-day ranking metric, so it uses only the ORDER of
# scores. A rank transform preserves every feature's ordering exactly,
# bounds all values to [0,1], and is immune to zero-inflation (ties at
# zero share a rank). It discards exactly the information the metric
# ignores.
#
# **Falsifiable prediction, recorded before running**: if the scaling
# hypothesis is right, rank scaling should (a) improve or at least not
# harm the baseline, and (b) crucially, make the DEEP-HISTORY columns
# helpful rather than neutral. If rank scaling is a null too, the
# sparse-column explanation is wrong and something else is limiting this
# model.
CANDIDATES = {
    "z-score scaling (current), 5yr history": dict(
        heads=3, gat_layers=1, negative_binomial=False, _scaling="zscore", _deep=False),
    "RANK scaling, 5yr history": dict(
        heads=3, gat_layers=1, negative_binomial=False, _scaling="rank", _deep=False),
    "RANK scaling, 9yr history (the key test)": dict(
        heads=3, gat_layers=1, negative_binomial=False, _scaling="rank", _deep=True),
}
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        scaling = window_config.pop("_scaling", "zscore")
        deep = window_config.pop("_deep", True)
        if not deep:
            keep = [i for i, c in enumerate(FEATURE_COLUMNS)
                    if c not in ("collision_count_2555d", "collision_count_3285d")]
            train_slice = [(x[:, :, keep], y, st) for x, y, st in train_slice]
            held_out_x_raw = held_out_x_raw[:, :, keep]

        train_x_seqs = [x for x, _, _ in train_slice]
        if scaling == "rank":
            train_instances = [(apply_rank_transform(x), y.T) for x, y, _ in train_slice]
            held_out_x = apply_rank_transform(held_out_x_raw)
        else:
        ...
        calib_x = apply_rank_transform(calib_x_raw) if scaling == "rank" else apply_feature_standardizer(calib_x_raw, mean, std)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "s6_rank_scaling_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "s6_rank_scaling_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "s6_rank_scaling_summary.csv")
