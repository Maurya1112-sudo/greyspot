...
logger = logging.getLogger("run_ucl_comparison_capacity")
...
# source alone.
CANDIDATES = {
    # Model CAPACITY (gat_hidden / gru_hidden) was tuned on the OSMnx
    # network and has never been re-swept on the real OS Open Roads
    # network - the same reasoning that justified re-sweeping head count
    # after the network swap. The real network has ~11k directed edges vs
    # OSMnx's ~7.5k and a different connectivity structure, so the
    # capacity that suited one is not automatically right for the other.
    # Current default is gat_hidden=16 / gru_hidden=32.
    "capacity: gat=16 gru=32 (current default)": dict(
        heads=3, gat_layers=1, negative_binomial=False, gat_hidden=16, gru_hidden=32,
    ),
    "capacity: gat=32 gru=64 (2x)": dict(
        heads=3, gat_layers=1, negative_binomial=False, gat_hidden=32, gru_hidden=64,
    ),
    "capacity: gat=8 gru=16 (0.5x - less overfitting at 6-11 instances)": dict(
        heads=3, gat_layers=1, negative_binomial=False, gat_hidden=8, gru_hidden=16,
    ),
    "capacity: gat=42 gru=42 (the paper's own stated hidden size)": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_capacity.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_capacity.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_capacity.csv")
