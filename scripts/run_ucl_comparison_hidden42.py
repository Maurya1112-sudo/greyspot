...
logger = logging.getLogger("run_ucl_comparison_hidden42")
...
# source alone.
CANDIDATES = {
    # gat_hidden=42 / gru_hidden=42 - the paper's OWN stated hidden size
    # ("The hidden units are set to 42", thesis Section 7.4.1), which
    # this project had only ever tested BUNDLED with their other
    # hyperparameters (notably epochs=20, which badly underfits this
    # project's feature set and confounded the whole bundle at 14.79%).
    # Isolated on the real network it is the best capacity found:
    # Lambeth 65.05% vs the 16/32 default's 63.59%, and - matching the
    # diagnosed failure mode - it HALVES the variance (std 5.22% vs
    # 10.28%) and lifts the worst window from 50.71% to 57.26%.
    #
    # p=0.61 on six windows, i.e. NOT significant alone. This run extends
    # it to the other two boroughs to reach n=18, which is gathering more
    # evidence to resolve an underpowered comparison - not re-testing
    # until a favourable p-value appears. The variance reduction is the
    # independently-diagnosed mechanism (AccHR@20 correlates r=0.731 with
    # crash density; the failing windows are the sparsest), which is why
    # this specific setting is worth the extra GPU time and the 2x/0.5x
    # variants (both worse) are not.
    "heads=3 + REAL OS network + hidden=42/42 (paper's own stated size)": dict(
        heads=3, gat_layers=1, negative_binomial=False, gat_hidden=42, gru_hidden=42,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_hidden42.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_hidden42.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_hidden42.csv")
