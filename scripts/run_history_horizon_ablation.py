...
logger = logging.getLogger("run_history_horizon_ablation")
...
# source alone.
# HISTORY-HORIZON DOSE-RESPONSE ABLATION, added 2026-09-04.
#
# Purpose: convert "we added some features and the score went up" into a
# measured dose-response curve, which is what makes the horizon claim a
# scientific finding rather than an engineering tweak. Each candidate is
# the SAME model with the SAME 35-column feature table; the only thing
# that varies is how deep the crash-history columns are allowed to reach.
# Every other column (geometry, POI, socio-demographic, AADF, calendar)
# is identical throughout, so the curve isolates horizon alone.
#
# A standalone-ranker version of this curve is already measured (history
# alone, no model: 30d 21.86% -> 1095d 70.15%, no plateau). This is the
# in-model version, which is what a reader actually needs.
HISTORY_LADDER = {
    "history <=30d (typical of the deep-learning literature)": ["collision_count_7d", "collision_count_14d", "collision_count_30d"],
    "history <=90d": ["collision_count_7d", "collision_count_14d", "collision_count_30d", "collision_count_90d"],
    "history <=365d (1 year)": ["collision_count_7d", "collision_count_14d", "collision_count_30d", "collision_count_90d", "collision_count_365d"],
    "history <=730d (2 years)": ["collision_count_7d", "collision_count_14d", "collision_count_30d", "collision_count_90d", "collision_count_365d", "collision_count_730d"],
    "history <=1095d (3 years)": ["collision_count_7d", "collision_count_14d", "collision_count_30d", "collision_count_90d", "collision_count_365d", "collision_count_730d", "collision_count_1095d"],
    "history <=1825d (5 years, final model)": ["collision_count_7d", "collision_count_14d", "collision_count_30d", "collision_count_90d", "collision_count_365d", "collision_count_730d", "collision_count_1095d", "collision_count_1825d"],
}
_ALL_HISTORY = HISTORY_LADDER["history <=1825d (5 years, final model)"]

CANDIDATES = {
    name: dict(heads=3, gat_layers=1, negative_binomial=False,
               feature_columns=[c for c in FEATURE_COLUMNS if c not in _ALL_HISTORY or c in keep])
    for name, keep in HISTORY_LADDER.items()
}
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        # `feature_columns` selects a subset of the SAME prebuilt instances by
        # column name, so every candidate sees identical data apart from the
        # history depth under test - no rebuild, no risk of the candidates
        # differing in anything else.
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "history_horizon_ablation_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "history_horizon_ablation_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "history_horizon_ablation_summary.csv")
