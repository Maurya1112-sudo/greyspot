...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_weather")
...
# source alone.
CANDIDATES = {
    "heads=3 + REAL OS network + POI + socio + WEATHER (re-test on the real network)": dict(
...
def build_instances(borough_name: str):
    ...
    table = attach_daily_weather_features(table, load_london_daily_weather(WEATHER_PATH))
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_weather.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_weather.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_weather.csv")
