"""Multi-window AccHR@20 evaluation testing `rate_link="exp"` - built
2026-09-02 after re-reading the paper's own linked reference repo
(github.com/STTDAnonymous/STTD) a second time, per the user's explicit
"connect every dot" instruction, found their mu/rate parameter uses a
log-link (raw model output exponentiated only inside the loss function)
rather than this project's original `softplus` (applied inside the
model itself, always non-negative even pre-loss) - see
`gat_temporal.py`'s `rate_link` parameter docstring for the full
derivation. This script tests whether adopting their exact activation
choice changes AccHR@20 versus the current best-known config
(`heads=3`, otherwise unchanged).
...
Run from the project root: python scripts/run_ucl_comparison_multiwindow_exp_link.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_exp_link")
...
CANDIDATES = {
    "heads=3 + rate_link=exp (paper's own reference-code log-link)": dict(
        heads=3, gat_layers=1, negative_binomial=False, rate_link="exp",
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_exp_link.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_exp_link.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_exp_link.csv")
