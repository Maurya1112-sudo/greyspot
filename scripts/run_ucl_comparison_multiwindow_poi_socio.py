"""Multi-window AccHR@20 evaluation WITH Point-of-Interest and
socio-demographic features - built 2026-09-02 after re-reading Gao et
al.'s PhD thesis a second time (the user's "think deeper" and "why don't
we get the same results if we copy their whole pipeline" prompts) found
their Table 7.2 feature set includes "PointofInterest" (Ordnance Survey,
20 classes) and "Socio-demographicCharacteristics" (Census 2011, 8
classes) for every borough - two real inputs this project had never
incorporated before now (weather, the third Table 7.2 input this
project was missing, was already added and found neutral in an earlier
pass).

Both are added here as disclosed, free substitutes for the paper's own
sources (see `ingest.poi` and `ingest.socio_demographic` for the full
reasoning): OSM POI tags (`shop`/`amenity`/`leisure`/`tourism`) in place
of OS's commercial Points of Interest product, and 2019 IMD deprivation-
domain scores + population density (already at 2011 LSOA geography,
the same as Census 2011) in place of a literal Census 2011 pull.

Reuses the same expanding-window walk-forward methodology as
`run_ucl_comparison_multiwindow.py` (see that file's original docstring
for why a single held-out window is too coarse to trust).

Known duplication (disclosed, not hidden): this script re-implements the
same feature-building steps as the other `run_ucl_comparison*.py`
scripts rather than importing a shared function - see those files' own
docstrings for why (a real, disclosed piece of tech debt, not hidden).

Run from the project root: python scripts/run_ucl_comparison_multiwindow_poi_socio.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_poi_socio")
...
CANDIDATES = {
    "heads=3 + POI + socio-demographic (Table 7.2 feature parity attempt)": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_poi_socio.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_poi_socio.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_poi_socio.csv")
