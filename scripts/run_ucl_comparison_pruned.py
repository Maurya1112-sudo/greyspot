...
logger = logging.getLogger("run_ucl_comparison_pruned")
...
# PRUNED variant, added 2026-09-03: drop the 5 casualty severity/type
# breakdown columns. Verified on real Lambeth data before building this
# experiment (per this project's own "verify on real data first" rule):
# these 5 columns are a decomposition of `collision_count` itself -
# correlated 0.14-0.85 with it - each 2-50x SPARSER than
# collision_count's own 0.021% nonzero rate (n_fatal: 0.00043%,
# n_serious: 0.00517%, n_slight: 0.01552%, n_pedestrian: 0.00561%,
# n_cyclist: 0.00905%). They fragment an already-rare signal into five
# noisier, more-collinear sub-columns rather than adding new
# information - a plausible source of overfitting noise distinct from
# every previous "add a new feature category" experiment, which only
# ever ADDED columns and never asked whether the EXISTING set carries
# redundant ones. This drops 30 -> 25 columns, touching nothing else.
PRUNED_FEATURE_COLUMNS = [
    c for c in FEATURE_COLUMNS
    if c not in {"n_fatal_casualties", "n_serious_casualties", "n_slight_casualties",
                 "n_pedestrian_casualties", "n_cyclist_casualties"}
]
...
# source alone.
CANDIDATES = {
    "current best (30 features, incl. 5 redundant casualty-severity columns)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=FEATURE_COLUMNS,
    ),
    "pruned (25 features, casualty-severity breakdown dropped)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=PRUNED_FEATURE_COLUMNS,
    ),
}
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        # `feature_columns`, when present, selects a subset of columns
        # (by name, against the FULL FEATURE_COLUMNS the instances were
        # built with) BEFORE standardisation - this is how the "pruned"
        # candidate drops the 5 redundant casualty-severity columns
        # without rebuilding the segment-day table twice.
        ...
        if wanted_cols is not None:
            ...
            train_slice_sel = [(x[:, :, col_idx], y, s) for x, y, s in train_slice]
            held_out_x_raw_sel = held_out_x_raw[:, :, col_idx]
        else:
            train_slice_sel = train_slice
            held_out_x_raw_sel = held_out_x_raw

        train_x_seqs = [x for x, _, _ in train_slice_sel]
        mean, std = fit_feature_standardizer(train_x_seqs)
        train_instances = [(apply_feature_standardizer(x, mean, std), y.T) for x, y, _ in train_slice_sel]
        held_out_x = apply_feature_standardizer(held_out_x_raw_sel, mean, std)
        ...
        calib_x_raw, calib_y, _ = train_slice_sel[-1]
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_pruned.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_pruned.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_pruned.csv")
