"""Architecture recheck ON THE REAL NETWORK - heads=3 was tuned entirely
on OSMnx-network data (both the original heads=1-vs-3 finding and the
later heads=1-4 sweep on top of POI+socio). Now that the real OS Open
Roads network is confirmed as this project's single biggest lever
(p=0.0010 across 3 boroughs - see `docs/decision_log.md`'s 2026-09-02
entries), the real network's different topology (denser, differently-
connected: 11,098 directed edges for Westminster vs OSMnx's 7,552)
could plausibly have a different optimal head count - genuinely
untested until now, not assumed.

Run on Lambeth first (not Westminster) - Lambeth is this project's one
open gap (63.59% vs the paper's 76.59%, the largest of the three
boroughs), so this is the borough where a genuine architecture
improvement would matter most, not just complete a sweep for its own
sake.
...
light protocol, POI+socio-demographic features) - only CANDIDATES
changes, sweeping heads=1/2/3/4 at gat_layers=1.

Run from the project root: python scripts/run_ucl_comparison_multiwindow_os_open_roads_headsweep.py
"""
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_headsweep")
...
# heads=1..4 at gat_layers=1, real OS Open Roads network + POI+socio
# features held constant - the untested axis this script exists to
# answer: is heads=3 still optimal on the real network's topology?
CANDIDATES = {
    "heads=1 + POI + socio-demographic, REAL OS Open Roads network": dict(heads=1, gat_layers=1, negative_binomial=False),
    "heads=2 + POI + socio-demographic, REAL OS Open Roads network": dict(heads=2, gat_layers=1, negative_binomial=False),
    "heads=3 + POI + socio-demographic, REAL OS Open Roads network": dict(heads=3, gat_layers=1, negative_binomial=False),
    "heads=4 + POI + socio-demographic, REAL OS Open Roads network": dict(heads=4, gat_layers=1, negative_binomial=False),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_headsweep.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_headsweep.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_headsweep.csv")
