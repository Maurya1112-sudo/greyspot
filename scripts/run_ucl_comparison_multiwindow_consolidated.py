"""Multi-window AccHR@20 evaluation on a CONSOLIDATED (coarser) road
network, on top of the confirmed POI+socio-demographic config - built
2026-09-02, the user's explicit "keep trying to beat their best model,
think for yourself" instruction, after this project's own Table 7.3
finding (see docs/decision_log.md) showed the AccHR@20 gap is most
plausibly structural (network/data), not decoder or architecture
choice - the paper's own OS Open Roads network is consistently ~1.5x
COARSER than this project's OSMnx network (Westminster: their 4,822
roads vs this project's 7,552 segments).

The real OS Open Roads swap remains blocked (the 2GB source GeoPackage
requires the user's own OS Data Hub account, no longer present
locally). **This script tests a self-directed, disclosed approximation
instead**: `ingest.network.consolidate_borough_graph` merges nearby OSM
intersection nodes by physical distance (`osmnx.simplification.
consolidate_intersections`), reducing segment count without needing any
new data source - NOT a claim of replicating OS Open Roads' actual
survey-based topology, just a genuinely testable version of "does a
coarser network help." A 2m tolerance was found (by direct search, not
derived) to bring Westminster to ~4,324 segments, the closest this
project could get to the paper's 4,822 - see that function's own
docstring for the full reasoning and its limits.

Every other setting (POI+socio-demographic features, heads=3) matches
the confirmed-significant configuration exactly - this script isolates
the network-granularity variable alone, on top of the already-best
config, not a fresh combination of untested changes at once.

Run from the project root: python scripts/run_ucl_comparison_multiwindow_consolidated.py
"""
...
from greyspot.ingest.network import build_borough_graph, consolidate_borough_graph, graph_to_edges_gdf, snap_collisions_to_graph, snap_points_to_graph  # noqa: E402
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_consolidated")
CONSOLIDATE_TOLERANCE_M = 2.0  # found by direct search - see consolidate_borough_graph's own docstring
...
# The confirmed-significant config (heads=3 + POI + socio-demographic)
# unchanged - this run isolates ONE new variable (network granularity)
# on top of it, not a fresh combination of untested changes at once.
CANDIDATES = {
    "heads=3 + POI + socio-demographic + consolidated network (~4,324 segments)": dict(
...
def build_instances(borough_name: str):
    ...
    graph = consolidate_borough_graph(
        graph, tolerance_m=CONSOLIDATE_TOLERANCE_M,
        cache_path=INTERIM_DIR / f"{borough_slug}_graph_consolidated_{CONSOLIDATE_TOLERANCE_M:.0f}m.graphml",
    )
    ...
    # ~75s for a borough) rather than re-downloaded on every run. A
    # SEPARATE cache file from the raw-network sweep's
    # `{borough_slug}_poi_counts.csv` - consolidation renumbers every
    # segment_id, so reusing that cache would silently key-mismatch
    # every row (found and fixed before running, not after - the wrong
    # cache would look like "no POIs anywhere," not crash loudly).
    poi_cache_path = INTERIM_DIR / f"{borough_slug}_poi_counts_consolidated_{CONSOLIDATE_TOLERANCE_M:.0f}m.csv"
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_consolidated.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_consolidated.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_consolidated.csv")
