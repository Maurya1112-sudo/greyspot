...
logger = logging.getLogger("run_ucl_comparison_longhistory")
...
# source alone.
CANDIDATES = {
    "heads=3 + POI + socio + LONG crash history (90d/365d added)": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_longhistory.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_longhistory.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_longhistory.csv")
