...
logger = logging.getLogger("run_ucl_comparison_multiyear")
...
# source alone.
CANDIDATES = {
    "best + MULTI-YEAR history (730d/1095d/1825d added)": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_multiyear.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_multiyear.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_multiyear.csv")
