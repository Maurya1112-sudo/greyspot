...
INPUT_WINDOW, HORIZON, STRIDE_DAYS = 14, 14, 90
...
# INPUT_WINDOW=14 variant (2026-09-01): the predecessor STZINB-GNN paper
# (Gao/Haworth/Zhuang et al., arXiv:2307.13816 - same research group, one
# generation earlier) explicitly reports only SYMMETRIC input/horizon
# configurations - "Long(14-14)" and "Short(7-7)" - never an asymmetric
# 20-in/14-out split. The later Tweedie paper's own "N=20" almost
# certainly reads as N_epoch=20 (training epochs), not input window
# length - `N` is defined earlier in that same paper as |V|, the road
# COUNT (in the thousands), which "N=20" cannot coherently mean - see
# docs/decision_log.md for the full reasoning. This script tests the
# family's own precedented symmetric 14-in/14-out convention instead of
# this project's prior (now-doubted) 20-in/14-out assumption, using the
# single best architecture lever found so far (heads=3).
CANDIDATES = {
    "heads=3, input_window=14 (symmetric, matches predecessor paper)": dict(heads=3, gat_layers=1, negative_binomial=False),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_inputwindow14.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_inputwindow14.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_inputwindow14.csv")
