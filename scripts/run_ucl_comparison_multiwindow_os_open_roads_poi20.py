...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_poi20")
...
# source alone.
CANDIDATES = {
    "heads=3 + REAL OS network + POI at 20-CLASS granularity (paper Table 7.2 parity) + socio": dict(
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_poi20.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_poi20.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_poi20.csv")
