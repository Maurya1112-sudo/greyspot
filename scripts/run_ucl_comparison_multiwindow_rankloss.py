...
CANDIDATES = {
    ...
    # 2026-09-01: heads=3 alone is the best architecture lever found
    # (Westminster 49.57%, Lambeth 47.31% mean across 6 windows each - a
    # real, replicated gain over the 42.25% baseline). This sweep adds
    # `pairwise_rank_hinge_loss` on top of heads=3 - a learning-to-rank
    # auxiliary loss that directly optimises "does a true-crash segment
    # score above non-crash segments," the exact quantity AccHR@20
    # measures, rather than only the distributional NLL fit (see
    # gat_temporal.py's pairwise_rank_hinge_loss docstring for the full
    # motivation). Two weights tested: a light touch (0.1) and a heavier
    # one (1.0), both at margin=1.0.
    "heads=3 + rank_loss(w=0.1)": dict(heads=3, gat_layers=1, negative_binomial=False, rank_loss_weight=0.1),
    "heads=3 + rank_loss(w=1.0)": dict(heads=3, gat_layers=1, negative_binomial=False, rank_loss_weight=1.0),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_rankloss.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_rankloss.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_rankloss.csv")
