...
logger = logging.getLogger("run_ucl_comparison_dense_training_same_windows")
...
def build_instances(borough_name: str):
    ...
    # EVAL grid - the light protocol's own stride, so the held-out
    # windows are the exact same dates as every other Lambeth run.
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_dense_training.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_dense_training.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_dense_training.csv")
