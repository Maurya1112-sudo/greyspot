...
logger = logging.getLogger("run_ucl_comparison_multiwindow_os_open_roads_encoder")
...
# source alone.
CANDIDATES = {
    # lr=0.001, NOT this project's usual 0.01. Established empirically
    # 2026-09-02: at lr=0.01 this encoder order diverges to NaN at epoch
    # 12 (localised to the GAT attention parameters, with gradient norms
    # of only ~0.003 - attention-softmax instability, not gradient
    # explosion); at lr=0.001 it trains stably for all 200 epochs. Our
    # 0.01 was tuned for the OPPOSITE encoder order, so testing their
    # topology at our learning rate would not be a fair test of the
    # topology itself.
    "PAPER encoder order: GRU->GAT (temporal_first), lr=1e-3": dict(
        heads=3, gat_layers=1, negative_binomial=False, encoder_order="temporal_first", lr=0.001,
    ),
    "PAPER encoder order + 2 GAT layers (their stated 2-layer GNN), lr=1e-3": dict(
        heads=3, gat_layers=2, negative_binomial=False, encoder_order="temporal_first", lr=0.001,
    ),
    # Control: the DEFAULT order at the same lr=1e-3, so any difference
    # is attributable to the encoder order rather than the learning rate.
    "CONTROL: default order (spatial_first) at the same lr=1e-3": dict(
        heads=3, gat_layers=1, negative_binomial=False, lr=0.001,
    ),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_os_open_roads_encoder.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_os_open_roads_encoder.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_os_open_roads_encoder.csv")
