...
logger = logging.getLogger("run_arch_key_replication")
...
# source alone.
# KEY ARCHITECTURE ARMS, REPLICATED ON A SECOND BOROUGH (2026-09-04).
#
# **Why.** The architecture ablation established this project's central
# claim - "topology is decisive, capacity is inert" - but it ran on
# LAMBETH ONLY at SEED 42. Two facts established today make that
# insufficient:
#   1. Single-seed noise is ~4 points (V8/V11), and the seed's bias is
#      borough-specific, flipping sign between boroughs.
#   2. THREE of three candidates that looked significant on one borough
#      failed cross-borough replication (hidden=42/42, the architecture
#      ensemble, weight_decay=0.01).
#
# Only the two LARGE effects are replicated here - layers (-26.46) and
# encoder order (-13.61) - because they are the ones the central claim
# rests on and the only ones comfortably clear of the noise band. The
# small effects (hidden -2.87, decoder -3.50) are BELOW the single-seed
# noise floor and are not worth replicating; they should be reported as
# "not distinguishable from seed noise" rather than as measured effects.
...
CANDIDATES = {
    ...
    "layers 1 -> 2 (their depth)": dict(_BASE, gat_layers=2),
    "encoder order -> temporal_first, at its own lr=5e-4": dict(_BASE, encoder_order="temporal_first", lr=5e-4),
}
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "arch_key_replication_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "arch_key_replication_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "arch_key_replication_summary.csv")
