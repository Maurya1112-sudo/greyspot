...
logger = logging.getLogger("run_c3_roadclass_replication")
...
# source alone.
# V7 - RE-VERIFY THE NULL LEDGER UNDER THE FINAL CONFIGURATION.
#
# **Why this is necessary.** ~20 nulls in this project were measured
# under the OLD configuration (30 features, 30-day history cap). V4
# already overturned TWO of them: "2 layers = null" became -26.46
# (p=0.0003), and "encoder order = equivalent" became -13.61 (p=0.0135).
# So the ledger demonstrably does not transfer.
#
# **Which null to re-test first, and why.** V10 showed that GEOMETRY +
# CRASH HISTORY is essentially the whole model (13 features == 35, and
# every other group combined is worth ~1 point). Road class
# (motorway / A road / B road / residential / ...) is a GEOMETRY-type
# feature, and it was tested once under the old config and found null
# (63.59% -> 63.74%, p=0.9725). Under a configuration where geometry
# dominates, it is the null most likely to have changed sign - and if it
# helps, it is nearly free (already present on every OS Open Roads edge).
#
# Tested against BOTH endpoints of the V10 ladder, since V10 showed
# feature-group effects are configuration-dependent and cannot be read
...
CANDIDATES = {
    "13 features (V10 best simple model)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=_L13),
    "13 features + road class (8 one-hot)": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=_L13 + ROAD_CLASS_COLUMNS),
    "35 features (FINAL)": dict(
        heads=3, gat_layers=1, negative_binomial=False, feature_columns=list(FEATURE_COLUMNS)),
    "35 features + road class": dict(
        heads=3, gat_layers=1, negative_binomial=False,
        feature_columns=list(FEATURE_COLUMNS) + ROAD_CLASS_COLUMNS),
}
...
def build_instances(borough_name: str):
    ...
    table = attach_road_class_features(table)
    ...
    instances = build_daily_multistep_instances(
        table, segment_order, list(FEATURE_COLUMNS) + ROAD_CLASS_COLUMNS, all_dates,
...
def evaluate_config(instances, edge_index, config: dict, n_windows: int) -> list[dict]:
    ...
    for held_out_idx in range(n - n_windows, n):
        ...
        if wanted_cols is not None:
            _ALL = list(FEATURE_COLUMNS) + ROAD_CLASS_COLUMNS
            col_idx = [_ALL.index(c) for c in wanted_cols]
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "c3_roadclass_per_window.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "c3_roadclass_summary.csv", index=False)
    logger.info("Written to %s", reports_dir / "c3_roadclass_summary.csv")
