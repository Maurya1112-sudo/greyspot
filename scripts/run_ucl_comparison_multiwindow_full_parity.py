...
from greyspot.ingest.poi import POI_FINE_COLUMNS, count_pois_near_segments_fine, download_borough_pois  # noqa: E402
...
logger = logging.getLogger("run_ucl_comparison_multiwindow_full_parity")
...
# source alone.
CANDIDATES = {
    "FULL Table 7.2 PARITY: REAL OS network + POI 20-class + socio 8 + road class 8 + date 4": dict(
...
def build_instances(borough_name: str):
    ...
    table = attach_date_features(table)
    ...
    poi_cache_path = INTERIM_DIR / f"{borough_slug}_poi_counts_os_open_roads_fine20.csv"
    if poi_cache_path.exists():
        poi_counts = pd.read_csv(poi_cache_path)
    else:
        pois = download_borough_pois(borough.osm_place)
        poi_counts = count_pois_near_segments_fine(pois, graph)
        poi_counts.to_csv(poi_cache_path, index=False)
        logger.info("Cached POI counts to %s", poi_cache_path)
    table = attach_poi_features(table, poi_counts, count_columns=POI_FINE_COLUMNS)
...
def main(borough_name: str = "Westminster") -> None:
    ...
    pd.DataFrame(all_rows).to_csv(reports_dir / "ucl_multiwindow_per_window_full_parity.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(reports_dir / "ucl_multiwindow_summary_full_parity.csv", index=False)
    logger.info("Written to %s", reports_dir / "ucl_multiwindow_summary_full_parity.csv")
